...
发行者发行存款盒子，设定好存款的必要参数和利息注入。在start-time之前注入设定的利息数量。到达start-time进入存款吸纳期。如果利息在start-time时不足存款盒子创建失败。
...
存款期间，如果发行存款设定中进行了存款凭证可转移的设定，那么用户可以自由交易自己手中的存款凭证。存款凭证到期后会自动取回本金和利息。用户在此阶段无法取回存入的通证。存款期结束，根据用户手中凭证的数量，自动兑换成本金和利息。
...
| 名称           | 类型 |      |      | 解释                                                         |
| -------------- | :--: | ---- | ---- | :----------------------------------------------------------- |
| --Bottom line  |      |      |      | (必填）整个存款协议发放利息所设定的最低总存款条件条件限定。不设置默认为0。设定范围在0～ceiling之间。 |
| --ceiling      |      |      |      | (必填）整个存款盒子最多容纳的存款数量。                      |
| --price        |      |      |      | (必填）每份所需所需要的存款。                                |
| -- transfer-on |      |      |      | (选填）用户存款后的存款凭证是否可以进行交易                  |
| --Interest     |      |      |      | (必填)利息的种类和数量                                       |
| --start-time   |      |      |      | (必填)用户可以存入存款盒子的时间。利息必须在接受存款时间之前，注入到存款盒子中。否则盒子创建失败。 |
| --Established  |      |      |      | (必填）存款开始计息的时间，                                  |
| --maturity     |      |      |      | (必填)存款到期交割本金和利息时间。                           |
